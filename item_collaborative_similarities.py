...
_all_movies = pd.read_sql('SELECT t.id, m.movielensid FROM trailers t JOIN movielens_movie m ON m.imdbidtt = t.imdbid WHERE t.best_file = 1 ORDER BY t.id', conn)
conn.close()
